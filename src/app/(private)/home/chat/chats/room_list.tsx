...
export default function RoomChat({ roomId }: { roomId: string }) {
    ...
    const [typingStatus, setTypingStatus] = useState<{ username: string, isTyping: boolean }[]>([]);
    const pollingIntervalRef = useRef<NodeJS.Timeout | null>(null);
    const typingTimeoutRef = useRef<NodeJS.Timeout | null>(null);
    const { typingStatuss, sendTypingStatus } = useTypingStatus(roomId, token);
    ...
    // В функции подключения WebSocket
    useEffect(() => {
        const connectWebSocket = () => {
            const socket = new SockJS(`${BACKEND_URL_CHAT}/ws`);
            ...
            client.connect({}, () => {
                ...
                client.subscribe(`/topic/room/${roomId}/typing`, (typing) => {
                    const status = JSON.parse(typing.body);
                    setTypingStatus(prev => [...prev, status]);
                });
            });
    ...
    const handleSendMessage = () => {
        ...
        stompClient.send(`/app/chat/${roomId}/send`, {}, JSON.stringify(messageDto));
        setNewMessage("");
    };
